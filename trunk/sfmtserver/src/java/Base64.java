...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    /**
     * Decodes the string and returns the string that was encoded
     *
     * @param encodedString The Base64 data to decode
     * @return the string which was encoded to retrieve encodedString
     */
     ...
     * @since 2.1
     */
    public static String encodeFromURL( URL url )
    {
        String encodedData = null;
        Base64.InputStream bis = null;
        try
        {
            ...
            // Set up some useful variables
            byte[] buffer = new byte[ Math.max((int)(uc.getContentLength() * 1.4),40) ]; // Need max() for math on small files (v2.2.1)
            int length   = 0;
            int numBytes = 0;
            ...
            bis = new Base64.InputStream(
                      new java.io.BufferedInputStream(
                      uc.getInputStream() ), Base64.ENCODE );
        ...
        {
            System.err.println( "Error encoding from url " + url );
        }   // end catch: IOException
        finally
        {
            try{ bis.close(); } catch( Exception e) {}
